...
module.exports = async (client, message) => {
	...
	if (client.basic.commands.huntbot.enable) {
			...
			.then((inUse) => {
				if (inUse) {
					...
					huntbotcaptchaprocess = client.childprocess.spawn("py", [
						path.join(__dirname, "./huntbot_captcha/huntbotcaptcha.py"),
					]);
		...
		process.on("exit", () => {
			client.logger.warn("Bot", "Huntbot", "Killing huntBot captcha solver...");
	...
	await channel
		...
		.then(async (clmsg) => {
			...
			async function getMessage() {
				return new Promise((resolve) => {
					...
					const timer = setTimeout(() => {
						client.logger.warn("Farm", "Checklist", "Rechecking checklist...");
						client.off("messageCreate", listener);
			...
			if (message == null) {
				...
				client.logger.alert("Farm", "Checklist", "Cannot retrieve checklist.");
				require("./function/farm.js")(client, message);
				return;
			}
