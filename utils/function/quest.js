...
module.exports = async (client, message) => {
    ...
    if (client.config.extra.enable && client.config.extra.token.length > 0) type = "duo";
    ...
    if (client.global.type == "Main") {
        ...
        extraSender = client.channels.cache.get(client.config.extra.autoquestchannelid);
        mainready = true;
    }
    if (client.global.type == "Extra") {
        extraclient = client;
        channel = client.channels.cache.get(client.basic.autoquestchannelid);
        mainSender = client.channels.cache.get(client.config.main.autoquestchannelid);
        extraready = true;
    }
                        ...
                        if (type == "duo" && !selectedQuest) {
                            switch (true) {
                                case quest.title.includes("Have a friend curse you"):
                                    questCurse(client, channel, quest, mainSender, extraSender);
                                    selectedQuest = true;
                                    break;
                                case quest.title.includes("Have a friend pray to you"):
                                    questPray(client, channel, quest, mainSender, extraSender);
                                    selectedQuest = true;
                                    break;
                                case quest.title.includes("Battle with a friend"):
                                    questBattle(client, channel, quest, mainSender, extraSender);
                                    selectedQuest = true;
                                    break;
                                ...
                                case quest.title.includes("Have a friend use an action command"):
                                    questActionMe(client, channel, quest, mainSender, extraSender);
                                    selectedQuest = true;
                                    break;
                                default: 
                                    break;
